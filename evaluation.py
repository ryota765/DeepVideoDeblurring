...
def load_batch_data(
    list_IDs_temp, data_dir, batch_size=16, dim=(128, 128), n_channels=3, n_stack=5
):
    ...
    for i, ID in enumerate(list_IDs_temp):
        ...
        for j in range(-(n_stack // 2), n_stack // 2 + 1):
            ...
            X_stack[
                :,
                :,
                n_channels * (j + n_stack // 2) : n_channels * (j + n_stack // 2 + 1),
            ] = np.load(
                os.path.join(input_data_dir, str(int(ID) + j).zfill(8) + ".npy")
            )
